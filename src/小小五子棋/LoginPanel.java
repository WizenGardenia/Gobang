...
public class LoginPanel extends javax.swing.JPanel {
	...
	private javax.swing.JButton machineButtonEasy;// 人机对战按钮
	private javax.swing.JButton machineButtonNormal;
	private javax.swing.JButton machineButtonHard;
	...
	private void initComponents() {
		...
		machineButtonEasy = new javax.swing.JButton();// 初始化人机对战按钮
		machineButtonNormal = new javax.swing.JButton();// 初始化人机对战按钮
		machineButtonHard = new javax.swing.JButton();// 初始化人机对战按钮
		...
		add(machineButtonEasy, gridBagConstraints);
		
		gridBagConstraints = new java.awt.GridBagConstraints();
		gridBagConstraints.insets = new Insets(0, 0, 0, 40);
		gridBagConstraints.gridx = 1;
		gridBagConstraints.gridy = 2;
		add(machineButtonNormal, gridBagConstraints);
		
		gridBagConstraints = new java.awt.GridBagConstraints();
		gridBagConstraints.insets = new Insets(0, 0, 0, 40);
		gridBagConstraints.gridx = 2;
		gridBagConstraints.gridy = 2;
		add(machineButtonHard, gridBagConstraints);
		...
		machineButtonEasy.setText("人机对战(简单)");
		machineButtonEasy.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				machinButtonEasyActionPerformed();
			}
		});
		
		machineButtonNormal.setText("人机对战(普通)");
		machineButtonNormal.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				machinButtonNormalActionPerformed();
			}
		});
		
		machineButtonHard.setText("人机对战(困难)");
		machineButtonHard.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				machinButtonHardActionPerformed();
			}
		});

		loginButton.setText("联机对战登录");
	...
	private void formMouseClicked(java.awt.event.MouseEvent evt) {
		JOptionPane.showMessageDialog(this, "请先登录！");// 弹出提示框
	}
	 ...
	 * 人机对战事件处理方法
	 */
	private void machinButtonEasyActionPerformed() {
	...
	private void machinButtonNormalActionPerformed() {
		MainFrame mainFrame = (MainFrame) getParent().getParent();// 获取主窗体的实例对象
		String name = nameTextField.getText(); // 获取用户昵称
		if (name.trim().isEmpty()) {// 如果用户没有输入名称
			JOptionPane.showMessageDialog(this, "请输入昵称");// 弹出对话框
	...
	private void machinButtonHardActionPerformed() {
		MainFrame mainFrame = (MainFrame) getParent().getParent();// 获取主窗体的实例对象
		String name = nameTextField.getText(); // 获取用户昵称
		if (name.trim().isEmpty()) {// 如果用户没有输入名称
			JOptionPane.showMessageDialog(this, "请输入昵称");// 弹出对话框
		...
		try {
			...
			if (socket.isConnected()) { // 如果连接成功
				...
				mainFrame.setSocket(socket); // 设置主窗体的Socket连接对象
				UserBean machine = new UserBean();// 创建AI机器人
				machine.setName("你已经死了");// 为机器人设置名称
				level=2;
				machine.setHost(InetAddress.getLocalHost()); // 初始化机器人IP
				machine.setTime(new Time(0));// 设置时间设为一个较大的，确保机器人选黑子
				mainFrame.setUser(user); // 将玩家放在窗体左边
