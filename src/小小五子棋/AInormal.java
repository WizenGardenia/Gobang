...
public class AInormal extends AI{
	...
	public AInormal(MainFrame outer) {
		frame = outer;// 外部窗体对象
		...
		for (int i = 0; i < 15; i++) {// 遍历棋盘行
			for (int j = 0; j < 15; j++) {// 遍历棋盘列
				if (chessmanArray[i][j] > 0) {// 如果此处有白棋子
					int tmp[] = catchChessModle(i, j, chessmanArray, (byte) 1);// 捕捉每个棋子形成的棋型
				...
				//优势值判定
				else if (chessmanArray[i][j] < 0) {// 如果此处有黑棋子
					int tmp[] = catchChessModle(i, j, chessmanArray, (byte) -1);// 捕捉每个棋子形成的棋型
					if (tmp[0] >= threat) {// 由于此时是AI落子，优先考虑进攻，其次是防守，故用大于等于
						threat = tmp[0];// 更新最大优势值
						x = tmp[1];// 更新落子横坐标
						y = tmp[2];// 更新落子纵坐标
					}
				}
	...
	private int[] catchChessModle(int x, int y, byte[][] chessmanArray,byte color) {
		...
		// 把参数点放入每行的中心部位
		if(chessmanArray[x][y]!=0)
			model[0][5] = model[1][5] = model[2][5] = model[3][5] = 1;
		else
