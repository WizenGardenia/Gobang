...
public class GobangPanel extends javax.swing.JPanel {
	...
	Deque<byte[][]> chessQueue = new LinkedList<byte[][]>(); // 游戏棋局的队列记录
	...
	private loopmusic FallingSound=new loopmusic();
	public static music WMusic=new music("src/res/music/victor.mp3");
	public static music LMusic=new music("src/res/music/loose.mp3");
	int m = 0,n=0;
	...
	public void paint(Graphics g1) {
		...
		if (!isStart()) { // 如果游戏不处于开始状态
			...
			if (towardsWin || win || draw) {
				...
				if(win) {
					if(WMusic.isAlive()==false) {
						if(m==0) {
							WMusic.start();		
						    m=1;
						}						
					}else {
						WMusic.Play("src/res/music/victor.mp3");					
					}
					
				}else{
					if(LMusic.isAlive()==false) {
						if(n==0) {
							LMusic.start();
						    n=1;
						}						
					}else {
						LMusic.Play("src/res/music/loose.mp3");					
					}
			    }
	...
	private void formMouseClicked(java.awt.event.MouseEvent evt) {
		...
		if (chessmanArray[xindex][yindex] == 0) {// 如果鼠标位置上没有棋子
			...
			FallingSound.playOnce("src/res/effect/chessVoice.wav");
			...
			if (winColor != 0 && winColor == myColor) {// 如果棋子胜利并且是和我的棋子颜色一致
				...
				String sql="insert into result values('"+User.getName()+"','"+User.getTime()+"','win')";
				MainFrame.SQL.update(sql);
	...
	public void chessForMachine(int xindex, int yindex) {
		...
		if (winColor != 0 && winColor == GobangModel.BLACK_CHESSMAN) {
			...
			MainFrame mainFrame = (MainFrame) getRootPane().getParent();
			UserBean User=mainFrame.getUser();
			String sql="insert into result values('"+User.getName()+"','"+User.getTime()+"','loose')";
			MainFrame.SQL.update(sql);
		}
	}
