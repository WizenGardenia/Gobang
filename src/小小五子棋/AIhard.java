...
public class AIhard extends AI{
	...
	private int[] forEach() {
		...
		int xb=-1,yb=-1;//防守棋临时坐标
		int xw=-1,yw=-1;//进攻棋临时坐标
		
		
		int threatb = 0;//玩家方威胁值
		int threatw = 0;//AI方优势值
		
		byte[][] chessmanArray = gobangModel1.getChessmanArray();// 获得棋盘
		for (int i = 0; i < 15; i++) {// 遍历棋盘行
			for (int j = 0; j < 15; j++) {// 遍历棋盘列
				if (chessmanArray[i][j] == 0) {// 如果此处没有棋子
					int tmp1[] = catchChessModle(i, j, chessmanArray, (byte) 1);// 捕捉每个棋子形成的棋型
					if (tmp1[0] > threatb) {// 如果存在比当前最大威胁值还要大的威胁值，则记录此处落子坐标
						threatb = tmp1[0];// 更新最大威胁值
						xb = tmp1[1];// 更新落子横坐标
						yb = tmp1[2];// 更新落子纵坐标
					}else if(tmp1[0] == threatb) {//出现同等威胁值时有概率更换落子点
					...
					//优势值判定
					int tmp2[] = catchChessModle(i, j, chessmanArray, (byte) -1);// 捕捉每个棋子形成的棋型
					if (tmp2[0] > threatw) {// 如果存在比当前最大优势值还要大的优势值，则记录此处落子坐标
						threatw = tmp2[0];// 更新最大优势值
						xw = tmp2[1];// 更新落子横坐标
						yw = tmp2[2];// 更新落子纵坐标
					}else if(tmp2[0] == threatw) {//出现同等优势值时有概率更换落子点
						int random = (int) (Math.random()*2);
						if(random==0)
						{
							threatw = tmp2[0];//更新最大优势
							xw = tmp2[1];// 更新落子横坐标
							yw = tmp2[2];// 更新落子纵坐标
						}
					}					
		...
		if(threatw>=threatb) {//优先进攻，己方优势高于对方时优先处理己方棋型
			x=xw;
			y=yw;
		}else {
			x=xb;
	 ...
	 */
	private int[] catchChessModle(int x, int y, byte[][] chessmanArray,byte color) {
	    ...
	    model[0][5] = model[1][5] = model[2][5] = model[3][5] = 0;
		// 以该棋子为中心，向两边走5步
		for (int i = 1; i <= 5; i++) {
			// 水平方向棋型
			if (x - i >= 0) {// 如果没有走出边界
				if(chessmanArray[x - i][y]==color)
					model[0][5 - i] = 1;// 将左侧棋子记录到水平棋型当中
				else if(chessmanArray[x - i][y]==0)
					model[0][5 - i] = 0;
			}
			if (x + i <= 14) {// 如果没有走出边界
				if(chessmanArray[x + i][y]==color)
					model[0][5 + i] = 1;// 将右侧棋子记录到水平棋型当中
				else if(chessmanArray[x + i][y]==0)
					model[0][5 + i] = 0;
			}
			...
			// 垂直方向棋型
			if (y - i >= 0) {// 如果没有走出边界
				if(chessmanArray[x][y - i]==color)
					model[1][5 - i] = 1;// 将上方棋子记录到垂直棋型当中
				else if(chessmanArray[x][y - i]==0)
					model[1][5 - i] = 0;
			}
			if (y + i <= 14) {// 如果没有走出边界
				if(chessmanArray[x][y + i]==color)
					model[1][5 + i] = 1;// 将下方棋子记录到垂直棋型当中
				else if(chessmanArray[x][y + i]==0)
					model[1][5 + i] = 0;
			}
			...
			if (x - i >= 0 && y + i <= 14) {// 如果没有走出边界
				if(chessmanArray[x - i][y + i]==color)
					model[2][5 - i] = 1;// 将左下方棋子记录到反斜棋型当中
				else if(chessmanArray[x - i][y + i]==0)
					model[2][5 - i] = 0;
			}
			if (x + i <= 14 && y - i >= 0) {// 如果没有走出边界
				if(chessmanArray[x + i][y - i]==color)
					model[2][5 + i] = 1;// 将右上方棋子记录到反斜棋型当中
				else if(chessmanArray[x + i][y - i]==0)
					model[2][5 + i] = 0;
			}
			...
			if (x - i >= 0 && y - i >= 0) {// 如果没有走出边界
				if(chessmanArray[x - i][y - i]==color)
					model[3][5 - i] = 1;// 将左上方棋子记录到正斜棋型当中
				else if(chessmanArray[x - i][y - i]==0)
					model[3][5 - i] = 0;
			}
			if (x + i <= 14 && y + i <= 14) {// 如果没有走出边界
				if(chessmanArray[x + i][y + i]==color)
					model[3][5 + i] = 1;// 将右下方棋子记录到正斜棋型当中
				else if(chessmanArray[x + i][y + i]==0)
					model[3][5 + i] = 0;
			}
		}
		...
		for (int i = 0; i < model.length; i++) {// 遍历棋型数组
			...
			index = getResult[0] - 5;
			if(index==0) {
				score=score+getResult[1];			
			}			
		}
		position[0] = score;// 记录此位置的综合评分（威胁值）
	...
	public int[] judgeModle(int model[]) {
		...
		for (int i = 0; i < library.length; i++) {// 遍历棋型库
			...
			if ((modelIndex = sb.indexOf(chessModel)) != -1) {// 如果存在此棋型，则将棋型出现的位置付给modelIndex
				...
				int half=modelIndex + stepIndex-5;
				if(half==0) {
	...
	private Object[][] getModelLibrary() {
		...
		Object[][] library = new Object[36][3];
		...
		library[0][1] = 9999;// 此棋型产生的威胁值（或优势值）
		...
		library[28][0] = deadthree1a;
		library[28][1] = 1999;
		library[28][2] = 4;
		library[29][0] = deadthree1b;
		library[29][1] = 1999;
		library[29][2] = 0;
		library[30][0] = livetwo;
		library[30][1] = 1499;
		library[30][2] = 6;
		library[31][0] = livetwo;
		library[31][1] = 1499;
		library[31][2] = 1;
		library[32][0] = deadtwo1a;
		library[32][1] = 149;
		library[32][2] = 3;
		library[33][0] = deadtwo1b;
		library[33][1] = 1499;
		library[33][2] = 4;
		library[34][0] = deadtwo1c;
		library[34][1] = 1499;
		library[34][2] = 0;
		library[35][0] = deadtwo1d;
		library[35][1] = 149;
		library[35][2] = 1;
